...
class zyz_XNet(nn.Module):
    ...
    def forward(self, input):
        ...
        output1 = self.DAB_Block_1(output1_0) # [1, 64, 128, 256]
        output1_cat = self.bn_prelu_2(torch.cat([output1, output1_0, down_2], 1))# [1, 131, 128, 256]
        ...
        out2_add = self.upsample_1(output2_cat)  # [1, 19, 128,256]
        out2 = self.upsample_4(out2_add)  # [1, 19, 256, 512]
        out1 = self.conv1(output1_cat)# [1, 19, 128, 256]
        out1 = out1 + out2_add# [1, 19, 128, 256]
        out1 = self.upsample_2(out1)  # [1, 19, 256, 512]
