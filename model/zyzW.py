...
class zyz_WNet(nn.Module):
    def __init__(self, classes=19, block_1=3, block_2=3, block_3=3):
        super().__init__()
        ...
        for i in range(0, block_2):
            self.DAB_Block_2.add_module("DAB_Module_2_" + str(i), DABModule1(128, d=[3, 7]))
            #self.DAB_Block_2.add_module("DAB_Module_2_" + str(i),
            #                            DABModule2(128, d=dilation_block_2[i]))
        self.bn_prelu_3 = BNPReLU(256 + 3)

        # DAB Block 3
        self.DAB_Block_3 = nn.Sequential()
        for i in range(0, block_3):
            self.DAB_Block_3.add_module("DAB_Module_3_" + str(i), DABModule1(128, d=[9, 11]))

        #self.classifier = nn.Sequential(Conv(259, classes, 1, 1, padding=0))

        self.eca_layer = eca_layer()
        self.psa1 = PSA_p(128,128)
    ...
    def forward(self, input):
        ...
        output2 = self.DAB_Block_2(output2_0)# [1, 128, 64, 128]
        output2 = self.psa1(output2_0)# [1, 128, 64, 128]
        output2 = self.DAB_Block_3(output2_0)# [1, 128, 64, 128]
        output2_cat = self.bn_prelu_3(torch.cat([output2, output2_0, down_3], 1))

        out1 = self.conv0(output1_cat)  # [1, 64, 128, 256]
        out1 = self.atten_sipath(out1)  # [1, 1, 128, 256]
        out21 = self.upsample_1(output2_cat)  # [1, 64, 128, 256]
        out2 = self.atten_cam(out21)  # [1, 64, 1, 1]
        out = out1 * out2  # [1, 64, 128, 256]
        out = out + out21  # [1, 64, 128, 256]
