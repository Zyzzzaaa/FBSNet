...
class SpatialAttention(nn.Module):#SAM,空间注意力机制,CBAM的下半部分
    ...
    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=False)#1，h,w
        max_out, _ = torch.max(x, dim=1, keepdim=False)#1,h,w
        x = torch.cat([avg_out, max_out], dim=1)#2,h,w
        x = self.conv1(x)#1,h,w
        return self.sigmoid(x)#c,h,w
...
class Xijie(nn.Module):#细节分支
    def __init__(self, nIn):
        super().__init__()
        #self.Conv1 = nn.Conv(nIn, nIn*2, kSize=3, stride=2, padding=1)
        self.up = nn.Conv2d(nIn,nIn*2,3,2,padding=1,groups=nIn)
        self.down = nn.Conv2d(nIn,nIn*2,3,2,padding=1,groups=1)
        self.bn_relu_1 = BNPReLU(nIn)
        self.bn_relu_2 = BNPReLU(nIn * 2)

    def forward(self, input):
        input = self.bn_relu_1(input)
        up1 = self.up(input)
        down1 = self.down(input)

        input = up1 + down1
        input = self.bn_relu_2(input)

        return input


class zyz_ANet(nn.Module):
    def __init__(self, classes=19, block_1=3, block_2=3):
        super().__init__()
        self.init_conv = nn.Sequential(
            Conv(3, 16, 3, 2, padding=1, bn_acti=True),
            Conv(16, 32, 3, 1, padding=1, bn_acti=True),
        ...
        # DAB Block 2
        dilation_block_2 = [[3,5,7],[5,9,11],[5,9,11]]#[[3,5,7],[3,5,7],[3,5,7],[5,9,11],[5,9,11],[5,9,11]]
        ...
        self.conv4 = Conv(259, classes, kSize=1, stride=1, padding=1)
        self.upsample_2 = UpsamplerBlock(259, 32)

        #细节分支定义函数
        self.conv3 = Conv(32, 64, kSize=3, stride=1, padding=1)
        self.atten_sipath = SpatialAttention()
        self.upsample_1 = UpsamplerBlock(128, 128)
        self.conv5 = Conv(160, 32, kSize=1, stride=1, padding=0)
    ...
    def forward(self, input):
        ...
        out = self.upsample_2(out)#1,32,256,512
        #out = F.interpolate(out, input.size()[2:], mode='bilinear', align_corners=False)

        #细节分支
        xj = self.conv3(output0)#64,256,512
        xj = self.xijie(xj)#128,128,256
        xj = self.atten_sipath(xj)#128,128,256
        xj = self.upsample_1(xj)#64,256,512
        xj = torch.cat([output0, xj], 1)#64+32,256,512
        xj = self.conv5(xj)#32,256,512

        #融合
        outlast = xj + out#32,256,512
        outlast = self.endatten(outlast)#32,256,512
        outlast = self.output_conv(outlast)#classes,512,1024
        outlast = F.interpolate(outlast, input.size()[2:], mode='bilinear', align_corners=False)
